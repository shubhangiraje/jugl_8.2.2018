app.controller('SearchRequestAddCtrl', function ($scope,jsonDataPromise, jsonPostDataPromise, $rootScope,
                                                 $timeout, modal, ChatUploader, uploadService, $log,gettextCatalog,$cordovaCamera,$interval) {
    ...
    if (!$scope.data) {
        ...
        $scope.data.onShow = function(kendoEvent) {
            ...
            $timeout(function(){
                ...
                jsonDataPromise('/ext-api-search-request/add', {ids: ids})
                    .then(function (res) {
                        ...
                        intervalSaveDraft=$interval(function(){
                            if($scope.data.isSaveDraft) {
                                if(!$rootScope.searchRequestDraftId) {
                                    jsonPostDataPromise('/ext-api-search-request-draft/save', {searchRequest: $scope.data.searchRequest}).then(function (res) {
                                        $rootScope.searchRequestDraftId = res.id;
                                    });
                                } else {
                                    jsonPostDataPromise('/ext-api-search-request-draft/update', {id: $rootScope.searchRequestDraftId, searchRequest: $scope.data.searchRequest}).then(function (res) {});
                                }
        ...
        $scope.data.addInterests = function() {
            ...
            kendo.mobile.application.navigate('view-interests-addstep1.html?type=addSearch');
        };
        ...
        $scope.data.save = function() {
            ...
            jsonPostDataPromise('/ext-api-search-request/save', params)
                .then(
                    function (res) {
