app.controller('InfoPopupOldCtrl', function ($scope,$rootScope,jsonDataPromise,jsonPostDataPromise,modal,$timeout,uploadService,status,gettextCatalog,$cordovaActionSheet,$cordovaCamera,$log, $cordovaFile, $cordovaFileTransfer) {
    ...
    if (!$scope.data) {
        ...
        $scope.data.addComment=function() {
            if(status.user.packet == 'VIP' || status.user.packet == 'VIP_PLUS') {
                $timeout(function(){
                    ...
                    jsonPostDataPromise('/ext-api-info-new/add-comment', {infoComment: $scope.data.infoComment.newComment, infoPopupData: $scope.data.info,infoCommentLanguage:$scope.data.infoComments.wikiCountry})
                        .then(function (data) {
        ...
        $scope.data.infoComments.load = function() {
			...
			$scope.countryIds='';
			angular.forEach($scope.data.infoComments.wikiCountry,function(item,index){
				if($scope.countryIds!==''){
            ...
            jsonDataPromise('/ext-api-info-new/list-comments', {
		...
		function updateCountryList(){
				jsonDataPromise('/ext-api-country/get-country-list-comments',{info_id: $scope.data.info.id})
						.then(function (res) {
						angular.extend($scope.data.infoComments.countryWikiArray, res);
						if(set_country_to_user===false){
									$scope.data.infoComments.wikiCountry = [{id:$rootScope.status.user.country_id}];
									set_country_to_user=true;
						}
		...
		$scope.$watch('data.infoComments.wikiCountry',function(newValue,oldValue) {
            if (newValue != oldValue) {
